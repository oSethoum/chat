	...
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		...
		case "usergroups", "userGroups":
			var (
				...
				query = &UserGroupQuery{config: gr.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
			gr.WithNamedUserGroups(alias, func(wq *UserGroupQuery) {
				*wq = *query
			})
		case "messages":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &MessageQuery{config: gr.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
			gr.WithNamedMessages(alias, func(wq *MessageQuery) {
				*wq = *query
			})
		}
	}
...
func newGroupPaginateArgs(rv map[string]interface{}) *groupPaginateArgs {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]interface{}:
			var (
				err1, err2 error
				order      = &GroupOrder{Field: &GroupOrderField{}}
			)
...
func (m *MessageQuery) collectField(ctx context.Context, op *graphql.OperationContext, field graphql.CollectedField, path []string, satisfies ...string) error {
	path = append([]string(nil), path...)
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		switch field.Name {
		case "group":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &GroupQuery{config: m.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
		...
		case "user":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &UserQuery{config: m.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
...
func newMessagePaginateArgs(rv map[string]interface{}) *messagePaginateArgs {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]interface{}:
			var (
				err1, err2 error
				order      = &MessageOrder{Field: &MessageOrderField{}}
			)
			if d, ok := v[directionField]; ok {
				err1 = order.Direction.UnmarshalGQL(d)
			}
	...
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		...
		case "usergroups", "userGroups":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &UserGroupQuery{config: u.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
			u.WithNamedUserGroups(alias, func(wq *UserGroupQuery) {
				*wq = *query
			})
		case "messages":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &MessageQuery{config: u.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
...
func (ug *UserGroupQuery) collectField(ctx context.Context, op *graphql.OperationContext, field graphql.CollectedField, path []string, satisfies ...string) error {
	path = append([]string(nil), path...)
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		switch field.Name {
		case "group":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &GroupQuery{config: ug.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
		...
		case "user":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &UserQuery{config: ug.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
...
func newUserGroupPaginateArgs(rv map[string]interface{}) *usergroupPaginateArgs {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]interface{}:
			var (
				err1, err2 error
				order      = &UserGroupOrder{Field: &UserGroupOrderField{}}
			)
			if d, ok := v[directionField]; ok {
				err1 = order.Direction.UnmarshalGQL(d)
			}
			...
			if err1 == nil && err2 == nil {
				args.opts = append(args.opts, WithUserGroupOrder(order))
			}
		case *UserGroupOrder:
			if v != nil {
				args.opts = append(args.opts, WithUserGroupOrder(v))
			}
		}
	}
