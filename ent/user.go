...
// QueryUserGroups queries the "userGroups" edge of the User entity.
...
func (u *User) NamedUserGroups(name string) ([]*UserGroup, error) {
	if u.Edges.namedUserGroups == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := u.Edges.namedUserGroups[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (u *User) appendNamedUserGroups(name string, edges ...*UserGroup) {
	if u.Edges.namedUserGroups == nil {
		u.Edges.namedUserGroups = make(map[string][]*UserGroup)
	}
	if len(edges) == 0 {
		u.Edges.namedUserGroups[name] = []*UserGroup{}
	} else {
		u.Edges.namedUserGroups[name] = append(u.Edges.namedUserGroups[name], edges...)
	}
}
