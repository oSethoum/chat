...
type GroupQuery struct {
	...
	withNamedUserGroups map[string]*UserGroupQuery
...
func (gq *GroupQuery) QueryUserGroups() *UserGroupQuery {
...
func (gq *GroupQuery) WithUserGroups(opts ...func(*UserGroupQuery)) *GroupQuery {
	query := &UserGroupQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withUserGroups = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withUserGroups; query != nil {
		if err := gq.loadUserGroups(ctx, query, nodes,
	...
	for name, query := range gq.withNamedUserGroups {
		if err := gq.loadUserGroups(ctx, query, nodes,
			func(n *Group) { n.appendNamedUserGroups(name) },
			func(n *Group, e *UserGroup) { n.appendNamedUserGroups(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range gq.withNamedMessages {
		if err := gq.loadMessages(ctx, query, nodes,
			func(n *Group) { n.appendNamedMessages(name) },
			func(n *Group, e *Message) { n.appendNamedMessages(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (gq *GroupQuery) loadUserGroups(ctx context.Context, query *UserGroupQuery, nodes []*Group, init func(*Group), assign func(*Group, *UserGroup)) error {
	...
	query.Where(predicate.UserGroup(func(s *sql.Selector) {
		s.Where(sql.InValues(group.UserGroupsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.group_user_groups
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_user_groups" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_user_groups" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (gq *GroupQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Group, init func(*Group), assign func(*Group, *Message)) error {
	...
	for _, n := range neighbors {
		fk := n.group_messages
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_messages" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_messages" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedUserGroups tells the query-builder to eager-load the nodes that are connected to the "userGroups"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithNamedUserGroups(name string, opts ...func(*UserGroupQuery)) *GroupQuery {
	query := &UserGroupQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	if gq.withNamedUserGroups == nil {
		gq.withNamedUserGroups = make(map[string]*UserGroupQuery)
	}
	gq.withNamedUserGroups[name] = query
	return gq
}

// WithNamedMessages tells the query-builder to eager-load the nodes that are connected to the "messages"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithNamedMessages(name string, opts ...func(*MessageQuery)) *GroupQuery {
	query := &MessageQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	if gq.withNamedMessages == nil {
		gq.withNamedMessages = make(map[string]*MessageQuery)
	}
	gq.withNamedMessages[name] = query
	return gq
}
