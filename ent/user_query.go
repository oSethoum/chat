...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	for name, query := range uq.withNamedUserGroups {
		if err := uq.loadUserGroups(ctx, query, nodes,
			func(n *User) { n.appendNamedUserGroups(name) },
			func(n *User, e *UserGroup) { n.appendNamedUserGroups(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadUserGroups(ctx context.Context, query *UserGroupQuery, nodes []*User, init func(*User), assign func(*User, *UserGroup)) error {
	...
	query.Where(predicate.UserGroup(func(s *sql.Selector) {
		s.Where(sql.InValues(user.UserGroupsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_user_groups
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_user_groups" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_user_groups" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) WithNamedUserGroups(name string, opts ...func(*UserGroupQuery)) *UserQuery {
	query := &UserGroupQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	if uq.withNamedUserGroups == nil {
		uq.withNamedUserGroups = make(map[string]*UserGroupQuery)
	}
	uq.withNamedUserGroups[name] = query
	return uq
}
