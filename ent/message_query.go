...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if mq.withGroup != nil || mq.withUser != nil {
		withFKs = true
	}
	if withFKs {
...
func (mq *MessageQuery) loadGroup(ctx context.Context, query *GroupQuery, nodes []*Message, init func(*Message), assign func(*Message, *Group)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_messages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
